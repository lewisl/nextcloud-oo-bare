...
- [x] `/etc/onlyoffice/documentserver/local.json` overrides:
  - Loopback binding (`127.0.0.1:8000`) (**Status:** yes)
  - SQL credentials matching PostgreSQL (user `oouser`) (**Status:** yes)
  - JWT secret applied to inbox/outbox/browser/session (**Status:** yes)
  - RabbitMQ URL present (**Status:** yes)
  - WOPI enabled (local.json wopi.enable=true) (**Status:** required for discovery)
- [ ] Ensure `local.json` keeps upstream keys (e.g., token enable structure) in sync with defaults (**Status:** verified after rewrite)
- [ ] DocumentServer log directories writable owned by `ds:ds` (**Status:** confirmed via service startup)
- [x] Health endpoint reachable: `curl http://127.0.0.1:8080/healthcheck` = 200 (**Status:** yes)
- [ ] Discovery endpoint reachable: `curl http://127.0.0.1:8080/hosting/discovery` = 404 (**Status:** resolved via wopi.enable=true)
...
- [ ] Nextcloud connector will need identical secret via `occ config:app:set onlyoffice jwt_secret` (**Status:** pending integration)
- [ ] Consider JWT header name defaults (`Authorization`) (**Status:** align during integration)

## 7. Nextcloud Application (pending)
- [ ] OnlyOffice connector app installed (`occ app:install onlyoffice`) (**Status:** not yet)
...
- [ ] `occ onlyoffice:documentserver --check` (or equivalent) passes
- [ ] Test doc creation/edit from browser: open `.docx`, `.xlsx`, `.pptx`, plain text
...
- [ ] UFW/iptables rules to enforce above (optional hardening) (**Status:** future hardening)
