"""Utilities for loading deployment parameters from configs/params.yaml."""
...
PARAMS_PATH = Path(__file__).resolve().parents[1] / "configs" / "params.yaml"
...
class Params:
    ...
    def validate(self) -> List[str]:
        ...
        if "@" not in self.deployment.admin_email:
            issues.append("deployment.admin_email must be a valid email address")
        for label, db in (
